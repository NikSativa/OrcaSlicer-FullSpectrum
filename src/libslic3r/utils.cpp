...
void set_log_path_and_level(const std::string& file, unsigned int level)
{
    ...
    // C:\\Users\\[user]\\AppData\\Local\\SnapOrka\\log\\*.log
    ...
    // SnapOrka: use SLIC3R_APP_KEY (= "SnapOrka") so the path matches the wx data_dir
    // path elsewhere, instead of hardcoding the legacy "Snapmaker_Orca" folder.
    if (const char *local_appdata = boost::nowide::getenv("LOCALAPPDATA"); local_appdata != nullptr && *local_appdata != '\0')
        log_folder = boost::filesystem::path(local_appdata) / SLIC3R_APP_KEY / "log";
#endif
