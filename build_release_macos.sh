...
function build_slicer() {
        ...
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            if [ "1." != "$BUILD_ONLY". ]; then
                cmake "${PROJECT_DIR}" \
                    ...
                    -DCMAKE_INSTALL_PREFIX="$PWD/SnapOrka" \
                    -DCMAKE_BUILD_TYPE="$BUILD_CONFIG" \
    ...
    (
        ...
        APP_SOURCE_PATH="../src$BUILD_DIR_CONFIG_SUBDIR/SnapOrka.app"
        ...
        cp -pR "$APP_SOURCE_PATH" "./SnapOrka.app"
        # fix resources
        resources_path=$(readlink "./SnapOrka.app/Contents/Resources")
        rm "./SnapOrka.app/Contents/Resources"
        cp -R "$resources_path" "./SnapOrka.app/Contents/Resources"
        # delete .DS_Store file
        find "./SnapOrka.app/" -name '.DS_Store' -delete
        ...
        APP_MACOS_DIR='./SnapOrka.app/Contents/MacOS'
        APP_FRAMEWORKS_DIR='./SnapOrka.app/Contents/Frameworks'
        EXECUTABLE="${APP_MACOS_DIR}/SnapOrka"
        ...
        if [ -f "${LIBSENTRY}" ]; then
            ...
            # Update rpath in SnapOrka to use @executable_path relative path
            if [ -f "${EXECUTABLE}" ]; then
                echo "Updating libsentry.dylib rpath in SnapOrka..."
        ...
        if [ -f "${APP_MACOS_DIR}/SnapOrka" ]; then
            echo "Generating dSYM for SnapOrka..."
            dsymutil "${APP_MACOS_DIR}/SnapOrka" -o "${DSYM_DIR}/SnapOrka.dSYM" 2>/dev/null || echo "Warning: Failed to generate dSYM for SnapOrka (no debug symbols?)"
        fi
    ...
    # fi
        ...
        # zip -FSr SnapOrka${ver}_Mac_${_ARCH}.zip OrcaSlicer.app
...
function build_universal() {
    ...
    mkdir -p "$PROJECT_BUILD_DIR/SnapOrka"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/SnapOrka/SnapOrka.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$PROJECT_DIR/build/arm64/SnapOrka/SnapOrka.app" "$UNIVERSAL_APP"
    
    # Get the binary path inside the .app bundle
    BINARY_PATH="Contents/MacOS/SnapOrka"
    
    # Create universal binary using lipo
    lipo -create \
        "$PROJECT_DIR/build/x86_64/SnapOrka/SnapOrka.app/$BINARY_PATH" \
        "$PROJECT_DIR/build/arm64/SnapOrka/SnapOrka.app/$BINARY_PATH" \
        -output "$UNIVERSAL_APP/$BINARY_PATH"
        
    echo "Universal binary created at $UNIVERSAL_APP"
    
    # Create universal crashpad_handler if both architectures have it
    CRASHPAD_ARM64="${PROJECT_DIR}/build/arm64/SnapOrka/SnapOrka.app/Contents/MacOS/crashpad_handler"
    CRASHPAD_X86="${PROJECT_DIR}/build/x86_64/SnapOrka/SnapOrka.app/Contents/MacOS/crashpad_handler"
    ...
    LIBSENTRY_ARM64="${PROJECT_DIR}/build/arm64/SnapOrka/SnapOrka.app/Contents/Frameworks/libsentry.dylib"
    LIBSENTRY_X86="${PROJECT_DIR}/build/x86_64/SnapOrka/SnapOrka.app/Contents/Frameworks/libsentry.dylib"
    ...
    # Update rpath in universal SnapOrka
    if [ -f "${LIBSENTRY_UNIVERSAL}" ]; then
        echo "Updating libsentry.dylib rpath in universal SnapOrka..."
    ...
    if [ -f "$PROJECT_DIR/build/arm64/SnapOrka/Snapmaker_Orca_profile_validator.app/Contents/MacOS/Snapmaker_Orca_profile_validator" ] && \
       [ -f "$PROJECT_DIR/build/x86_64/SnapOrka/Snapmaker_Orca_profile_validator.app/Contents/MacOS/Snapmaker_Orca_profile_validator" ]; then
        echo "Creating universal binary for Snapmaker_Orca_profile_validator..."
        UNIVERSAL_VALIDATOR_APP="$PROJECT_BUILD_DIR/SnapOrka/Snapmaker_Orca_profile_validator.app"
        rm -rf "$UNIVERSAL_VALIDATOR_APP"
        cp -R "$PROJECT_DIR/build/arm64/SnapOrka/Snapmaker_Orca_profile_validator.app" "$UNIVERSAL_VALIDATOR_APP"
        ...
        # Create universal binary using lipo
        lipo -create \
            "$PROJECT_DIR/build/x86_64/SnapOrka/Snapmaker_Orca_profile_validator.app/$VALIDATOR_BINARY_PATH" \
            "$PROJECT_DIR/build/arm64/SnapOrka/Snapmaker_Orca_profile_validator.app/$VALIDATOR_BINARY_PATH" \
    ...
    DSYM_DIR="$PROJECT_BUILD_DIR/SnapOrka/dSYM"
    mkdir -p "${DSYM_DIR}"
    
    # Generate dSYM for universal main app
    if [ -f "$UNIVERSAL_APP/$BINARY_PATH" ]; then
        echo "Generating dSYM for universal SnapOrka..."
        dsymutil "$UNIVERSAL_APP/$BINARY_PATH" -o "${DSYM_DIR}/SnapOrka.dSYM" 2>/dev/null || echo "Warning: Failed to generate dSYM for universal SnapOrka"
    fi
